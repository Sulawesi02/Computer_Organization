...
//---------------------------------{译码}begin------------------------------------//
    ...
    wire inst_SGT , inst_XNOR , inst_LLI;
    ...
    assign inst_SGT   = op_zero & sa_zero    & (funct == 6'b000001);// 大于则置位
    assign inst_XNOR  = op_zero & sa_zero    & (funct == 6'b011011);// 逻辑同或运算
    assign inst_LLI   = (op == 6'b010101);                  // 立即数装载低半字节
    ...
    // 传递到执行模块的ALU源操作数和操作码
    wire inst_add, inst_sub, inst_slt, inst_sltu, inst_sgt;
    wire inst_and, inst_nor, inst_or,  inst_xor,  inst_xnor;
    wire inst_sll, inst_srl, inst_sra, inst_lui,  inst_lli;
    assign inst_add = inst_ADDU | inst_ADDIU | inst_LW | inst_SW; // 做加法运算 指令
    ...
    assign inst_sgt = inst_SGT;  // 大于则置位
    assign inst_xnor= inst_XNOR; // 逻辑同或运算
    assign inst_lli = inst_LLI;  // 立即数装载低半字节
...
//---------------------------------{写回}begin------------------------------------//
    ...
    assign inst_wdest_rd = inst_ADDU | inst_SUBU | inst_SLT | inst_AND | inst_NOR
                          | inst_OR   | inst_XOR  | inst_SLL | inst_SRL | inst_SGT | inst_XNOR;                   
    // 寄存器堆写使能信号，非复位状态下有效
